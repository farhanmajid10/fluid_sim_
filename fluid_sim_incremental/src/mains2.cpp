...
//    GLuint densityTexture;
    ...
    Shader advectionShader;  //moves density along velocity.

    GLuint densityTextures[2]; //double buffer for pingpong. reads from one and writes to the other forever.
    GLuint velocityTexture; //stores velocity field in vx and vy.

    GLuint fbo[2]; //framebuffers are connected to each of the density textures.
...
void createVelocityTexture() {
    ...
    void createDensityTextures(){
        ...
        //4 blobs   
        addBlob(initialData, NX/4,NY/2, 20,2.0f);  // takes in x-coord, y-coord, radius and brightness.
        addBlob(initialData, 3*NX/4, NY/2, 20,2.0f);
        addBlob(initialData, NX/2,NY/4, 15,1.5f);
        addBlob(initialData, NX/2,3*NY/4, 15,1.5f);
        
        for(int i = 0; i < 2; i++){
            glGenTextures(1,&densityTextures[i]); // generates a texture object name and writes to &desnsityTextures[i]
            glBindTexture(GL_TEXTURE_2D, densityTextures[i]);  //binds the texture so that all texture functions go to this binded texture until something else is binded or its unbinded.
            glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, NX, NY, 0, GL_RED, GL_FLOAT, initialData);  // gets texture type, base optimization level, GL_redchannel_32bitfloating point number, x-coord, y-coord, border_val, channel, element_type, pointer to CPU pixels.
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); //sets minification filter.
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // sets magnification filter.
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); 
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        }
    ...
    void createFramebuffers(){
        for(int i = 0; i < 2; i++){
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, densityTextures[i], 0);  //targetbuffer, attachmentpoint, texture type, texture ID, optimization lvl
